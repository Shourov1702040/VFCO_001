...
def index_alloc(csv_filename, total_clients, Total_data, data_scale):
    with open(csv_filename, 'w', newline='') as f:
        ...
        all_r_values = [f'R-{i+1}' for i in range(1, Total_data + 1)]
...
def generate_challenge(edge_info, Data_replicas):
    ...
    for i in range(len(Data_replicas)):
        ...
        tree = build_MHT(leafs, replica_ids[i], shuffle_key, sec_code)
        Trees.append(tree)
        block_loc_key[f"R-{i+1}"] = loc_key
        Roots.append(tree[-1])

    G_leafs = resequence_nodes(Roots, "G")
    G_Tree = build_MHT(G_leafs, "G", shuffle_key, sec_code)
    challenge_all =[]
    proof_all = {}
    loc_key_all = {}
